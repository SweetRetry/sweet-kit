...
- Database 切面放在 `packages/database`，使用 Drizzle ORM、SQLite 与 `better-sqlite3`；Better Auth 通过官方 Drizzle adapter 使用同一数据库实例和 schema。
...
- SQLite 适合作为开发套件的本地可运行基线；如果后续部署目标要求 PostgreSQL，应通过新的 ADR 变更 database driver 和 migration 策略。
