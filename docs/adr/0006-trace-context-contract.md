...
- **改用 `server-timing: trace;desc=<traceparent>`**：这是 trace-context 仓库当前草案给出的 response binding，浏览器可经 `PerformanceServerTiming` 读取。但面向 API 消费者更难取用，且草案状态不稳定。放弃。
...
- 客户端从 `traceparent` 取 trace id，W3C 格式的解析由 `packages/request` 封装，调用方不直接处理。
